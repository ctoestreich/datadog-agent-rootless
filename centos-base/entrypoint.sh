...
if [ "$DD_HOSTNAME" ]; then
  sed -i -e "s/%DD_HOSTNAME%/$DD_HOSTNAME/g" ${DD_ETC_ROOT}/datadog.conf
fi

if [ "$API_KEY" ]; then
  sed -i -e "s/%API_KEY%/$API_KEY/g" ${DD_ETC_ROOT}/datadog.conf
fi

if [ "$DD_TAGS" ]; then
  sed -i -e "s/%DD_TAGS%/$DD_TAGS/g" ${DD_ETC_ROOT}/datadog.conf
else 
  sed -i -e "s/%DD_TAGS%/fixme,environemnt:ignore/g" ${DD_ETC_ROOT}/datadog.conf
fi

if [ "$DD_URL" ]; then
  sed -i -e "s/%DD_URL%/$DD_URL/g" ${DD_ETC_ROOT}/datadog.conf
else
  sed -i -e "s/%DD_URL%/http:\/\/app.datadoghq.com/g" ${DD_ETC_ROOT}/datadog.conf
fi

echo "${DD_ETC_ROOT}/datadog.conf"
echo ""
cat ${DD_ETC_ROOT}/datadog.conf
echo ""
